...
function Struct() {
    ...
    this.word8Sle = this.word8Sbe = this.word16Sle = this.word16Sbe = this.word16Ule = this.word16Ube =
        this.word32Sle = this.word32Sbe = this.word32Ule = this.word32Ube =
            this.word64Sle = this.word64Sbe = this.word64Ule = this.word64Ube = function (name) {
            };
    ...
    this.getObject = function () {
        var result = {}
        for (field_name in priv.fields) {
            result[field_name] = priv.fields[field_name].get()
            if (result[field_name] instanceof Struct)
                result[field_name] = result[field_name].getObject()
        }
        return result
    }
